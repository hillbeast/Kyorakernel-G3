/*
 *  linux/drivers/mtd/onenand/s5pc110.c
 *
 *  OneNAND driver for S5PC110 (AUDI Controller)
...
#define DRIVER_NAME	"s5p6442-onenand"
...
// To Do: make platform independent code
#define ONENAND_PHYS_BASE			(0xB0000000)
 ...
 */
static unsigned short onenand_readw(const volatile void __iomem  *addr)
{
	return readw(addr);
}
 ...
 */
static void onenand_writew(unsigned short value, const volatile void __iomem *addr)
{
	writew(value, addr);
}
...
/**
 * onenand_dma_start - [Internal] DMA transfer
 * @param mtd		MTD data structure
 ...
 * Assumption:
 */
static int onenand_dma_start(struct mtd_info *mtd, void __iomem *src, void __iomem *dest, u32 length, u32 direction)
{
	struct onenand_chip *this = mtd->priv;
	...
	{
		writel((4 << 16) | (0 << 8) | (1 << 0),		/* 16-Burst, Incremental, 16-bit */
				CTRL_DMA_BASE + CTRL_DMA_SRC_CFG_OFFSET);
		writel((4 << 16) | (0 << 8) | (2 << 0),		/* 16-Burst, Incremental, 32-bit */
...
/**
 * onenand_dma_wait - [Internal] DMA transfer
 * @param mtd		MTD data structure
 *
 * Transfer data through DMA
 * Assumption:
 */
int onenand_dma_wait(struct mtd_info *mtd)
{
	struct onenand_chip *this = mtd->priv;
	void __iomem *CTRL_DMA_BASE = this->base + ONENAND_CTRL_OFFSET;
	u32 status;
 ...
 */
int onenand_read_bufferram(struct mtd_info *mtd, int area,
		unsigned char *buffer, int offset, size_t count)
{
	...
	{
		//if ((u32)buffer & 0xFFF)	// buffer is not aligned to page size (4KB).
		if ((u32)buffer & (mtd->writesize - 1))
		{
			// To Do:
	...
	/* DMA transfer */
	{
		...
		// To Do :
		//    Check whether cache flush (or clean) operation can be removed.
		//    If so, we can increase read performance.
		//dmac_flush_range(buffer, buffer + count);
		//dmac_clean_range(buffer, buffer + count);
		onenand_dma_start(mtd, src_addr, dest_addr, count, DMA_IN2OUT);
		dmac_inv_range(buffer, buffer + count);
		onenand_dma_wait(mtd);
	}
		...
		const unsigned char *buffer, int offset, size_t count)
{
	...
	/* DMA Transfer */
	{
		...
		dest_addr = (void __iomem *)ONENAND_PHYS_BASE + area;
		dest_addr += onenand_bufferram_offset(mtd, area) + offset;

		dmac_flush_range(buffer, buffer + count);
		onenand_dma_start(mtd, src_addr, dest_addr, count, DMA_OUT2IN);
		onenand_dma_wait(mtd);
	}
...
static int onenand_probe(struct mtd_info *mtd)
{
	...
	if (syscfg & ONENAND_SYS_CFG1_SYNC_READ) {
		printk(KERN_INFO "OneNAND Sync. Burst Read\n");
		...
		printk(KERN_INFO "OneNAND Async. Read\n");
...
static int __devinit s5p6442_onenand_probe(struct platform_device *dev)
{
	...
	printk(KERN_INFO "OneNAND: s5p6442_onenand_probe - (Virt) Base Address: 0x%08X\n",
	...
	//if (pdata->parts != NULL)
	if (1)
		err = add_mtd_partitions(&info->mtd, s3c_partition_info, ARRAY_SIZE(s3c_partition_info));
	else
#endif
...
/* PM Support */
#ifdef CONFIG_PM
static int s5p6442_onenand_suspend(struct platform_device *pdev, pm_message_t pm)
{
...
static int s5p6442_onenand_resume(struct platform_device *pdev)
{
	struct onenand_info *info = dev_get_drvdata(&pdev->dev);
...
#else
#define s5pc110_onenand_suspend		NULL
#define s5pc110_onenand_resume		NULL
#endif	// end of #ifdef CONFIG_PM

static int __devexit s5p6442_onenand_remove(struct platform_device *dev)
{
	//struct platform_device *pdev = to_platform_device(dev);
	struct platform_device *pdev = dev;
	struct onenand_info *info = dev_get_drvdata(&pdev->dev);
...
static struct platform_driver s5p6442_onenand_driver = {
	.probe		= s5p6442_onenand_probe,
	.remove		= __devexit_p(s5p6442_onenand_remove),
	.suspend	= s5p6442_onenand_suspend,
	.resume		= s5p6442_onenand_resume,
	.shutdown	= NULL,		// TO DO
	.driver		= {
		.name	= DRIVER_NAME,
		.owner	= THIS_MODULE,
...
static int __init s5p6442_onenand_init(void)
{
	printk(KERN_INFO "S5P6442 OneNAND Driver.\n");

	return platform_driver_register(&s5p6442_onenand_driver);
}

static void __exit s5p6442_onenand_exit(void)
{
	platform_driver_unregister(&s5p6442_onenand_driver);
}

module_init(s5p6442_onenand_init);
module_exit(s5p6442_onenand_exit);
