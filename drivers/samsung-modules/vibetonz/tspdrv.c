...
/* 
 * VIBRATOR_PERIOD is the "Down_Counter Value",VIBRATOR_ON_PERIOD is the "Compare Value" . 
 * After timer starts down-counter counts down and when down-counter value equals VIBRATOR_ON_PERIOD the PWM output changes from LOW to HIGH
 * Percent_Of_Duty_Cycle 	= (VIBRATOR_ON_PERIOD/VIBRATOR_PERIOD)
 * Percent_Of_Duty_Cycle	= 1%	Full Driving or Braking (From ISA1000 Approval Sheet)
 *				= 50%	No Vibration
 *				= 99%	Full Driving
 */

/*
 * Tunning for 22.4KHz(Optical Value for a Cf connected to the vibrator) PWM Output
 * VIBRATOR_PERIOD depends on the PCLK,(this value can be read from the file /plat-s5p64xx/pwm-s5p6442.c in func s5p6442_timer_setup())
 * Counter_Timeperiod = (1/(PCLK/4))
 * VIBRATOR_PERIOD    = (1/((Counter_Timeperiod)*(22.4KHz)))
 *		              = (PCLK/4)/(22.4KHz)
 * VIBRATOR_ON_PERIOD = (Percent_Of_Duty_Cycle)*(VIBRATOR_PERIOD)
 */
 
/* Output gain can also be controlled by Rf connected to the vibrator */
...
/*Depending on the pwm_duty sets the vibrator_period and vibrator_on_period*/
static unsigned long old_duty =0;
...
static void set_vib_period(unsigned long clk_rate )
{	
	mutex_lock(&vib_mut_lock);
	vibrator_period 	= ((clk_rate)/(89600)) ;	
	vibrator_on_period  = (vibrator_period*vib_pwm_duty)/100 +1;					
	mutex_unlock(&vib_mut_lock);	
}

/*
 * With DVFS enabled for Application Processor(AP SoC), PCLK-->input to the PWM module also changes, so correspondingly VIBRATOR_PERIOD,VIBRATOR_ON_PERIOD
 * should also be changed for maintaining the PWM output frequency at 22.4KHz.
 * Registering with DVFS notifier will notify the vibetonz driver whenever the DVFS is ON.
 */
#ifdef CONFIG_CPU_FREQ
static struct notifier_block   freq_transition;
static unsigned long old_clkrate = 0;
...
static inline int s5p_vibetonz_register_cpufreq(void)
{
    freq_transition.notifier_call = s5p_vibetonz_cpufreq_transition;
    return cpufreq_register_notifier(&freq_transition,
                                         CPUFREQ_TRANSITION_NOTIFIER);
}

static inline void s5p_vibetonz_deregister_cpufreq(void)
{
...
static ssize_t vibrator_show_pwm_duty(struct device *dev,struct device_attribute *attr, char *buf)
{ 	
    return sprintf(buf, "%lu\n", vib_pwm_duty);
}
static ssize_t vibrator_store_pwm_duty(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{      
		int ret;
		unsigned long duty;
		ret=0;
		if((strict_strtoul(buf,10,&duty)<0) || (duty > 99))
			return -EINVAL;
		set_vib_pwm_duty(duty);		
		return count;
}
static DEVICE_ATTR(pwm_duty, S_IRUGO | S_IWUGO, vibrator_show_pwm_duty, vibrator_store_pwm_duty);

/*For testing*/
static ssize_t vibrator_show_test(struct device *dev,struct device_attribute *attr, char *buf)
{ 	
	printk(" [VIBETONZ] %s \n",__func__);
...
static DEVICE_ATTR(test, S_IRUGO | S_IWUGO, vibrator_show_test, vibrator_store_test);
...
static int vibetonz_start(void)
{
	...
	if(!vib_wq){
		printk("Not enough memory for vibrator workqueue \n");
		ret = -ENOMEM;
		goto ERR_EXIT;
	}	
	INIT_WORK(&work_vib,vib_work_func);	
	
	/*Registering the callback notification for DVFS*/
	s5p_vibetonz_register_cpufreq();
	
	/* timed_output_device settings */
	ret = timed_output_dev_register(&timed_output_vt);
	if(ret)
	{
		printk(KERN_ERR "[VIBETONZ] timed_output_dev_register failed \n");
		goto ERR_EXIT;
	}
	
	/*sysfs attribute for controlling vibration pwm duty*/
	vibrator_class = class_create(THIS_MODULE, "vibrator");
	if (IS_ERR(vibrator_class))
	{
		printk("Failed to create class(vibrator)!\n");
		goto ERR_EXIT;
	}
	vib_device = device_create(vibrator_class, NULL, 0, NULL, "vib_device");
	if (IS_ERR(vib_device))
	{
		printk("Failed to create device(vib_device)!\n");
		goto ERR_DESTROY_CLASS;
	}
	
	if (device_create_file(vib_device, &dev_attr_pwm_duty) < 0)
	{
		printk("Failed to create device file(%s)!\n", dev_attr_pwm_duty.attr.name);		
		goto ERR_DESTROY_DEVICE;
	}
	
	/*For testing*/
	if (device_create_file(vib_device, &dev_attr_test) < 0)
	{
...
ERR_DESTROY_DEVICE:
	device_destroy(vibrator_class,0);
ERR_DESTROY_CLASS:
...
static int ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	//	printk("[VIBETONZ] stop_kernel_timer \n");
            /* 
	...
	//	printk("[VIBETONZ] enable_amp \n");
	    	wake_lock(&vib_wakelock);
            	ImmVibeSPI_ForceOut_AmpEnable(arg);
            	break;

        case TSPDRV_DISABLE_AMP:
	//	printk("[VIBETONZ] disable_amp \n");
            	ImmVibeSPI_ForceOut_AmpDisable(arg);
	    	wake_unlock(&vib_wakelock);
            	break;
