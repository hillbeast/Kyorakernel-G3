...
#define ADC_START(r,p)   (r) | S3C_ADCCON_PRSCEN | S3C_ADCCON_PRSCVL(p) | \
...
#define WAIT_EVENT_TIMEOUT		(HZ/50)					/* 20ms */
#define ADC_RETRY_NUM			0					/* no retry */
#define SLEEP_TIMEOUT			(HZ/5)					/* 200ms */
...
static inline void start_hw_adc (int channel)
{
	...
	writel (channel, base_addr + S3C_ADCMUX);  // channel muxing (s5p6442 has separate register for muxing)
	writel (ADC_START(adc[channel].resol, adc[channel].presc), base_addr+S3C_ADCCON);
}
...
#ifdef S3C_ADCTS_DEBUG
	printk("%s: value= %d (channel : %d)\n", __func__, adc_value[channel], channel);
#endif
